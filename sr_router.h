...
void ForwardPacket(struct sr_instance * sr,struct sr_rt* newInterface,struct sr_ethernet_hdr * header,uint8_t * packet,unsigned int len,struct ip * ipheader, struct cache * node,bool);
struct cache * getNode(uint32_t ip);
void sendARPrequest(struct sr_instance * sr, uint32_t dst_ip, char* interface,struct sr_rt* newInterface,bool);
